    ...
    export default {
        ...
        methods: {
            ...
            handleDelete(index, row) {
                this.attentionIndex = index;
                this.attentionName = row.username;
                this.delVisible = true;
            },
            // 确定删除
            deleteRow() {
                console.log(this.attentionName);
                var params = {
                    'data': {
                        'user_name': localStorage.getItem('ms_username'),
                        'attention_name': this.attentionName
                    }
                }
            ...
            getAttentionList() {
                ...
                api.getAttentionList(params).then((res) => {
                    if (res.data.status) {
                        this.totalNum = res.data.info.totalNum
                        this.attentionList = res.data.info.list;
                        //初始avatarURL
                        let token = tripledesUtil.decrypt(localStorage.getItem('token'));
                        for (let i = 0; i < this.attentionList.length; i++) {
                            this.attentionList[i].avatar = this.attentionList[i].avatar + '?token=' + token;
                        }
